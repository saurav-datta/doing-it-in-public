---
title: papers and blogs
draft: false
tags:
  - papers-and-blogs
description: Papers and blogs
---

- [LLM arch](#llm-arch)
- [Embeddings](#embeddings)
- [RAG](#rag)
- [Database](#database)
- [Apple FM](#apple-fm)
- [JEPA](#jepa)

# LLM arch
- [Beyond GPT architecture: Why Google's Diffusion approach could reshape LLM deployment | VentureBeat](https://venturebeat.com/ai/beyond-gpt-architecture-why-googles-diffusion-approach-could-reshape-llm-deployment/)
- [How we built our multi-agent research system \ Anthropic](https://www.anthropic.com/engineering/built-multi-agent-research-system)
- [Personalization of Large Language Models: A Survey](https://openreview.net/pdf?id=tf6A9EYMo6)

# Embeddings
[Vec2Vec: A Compact Neural Network Approach for Transforming Text Embeddings with High Fidelity](https://arxiv.org/abs/2306.12689)

# RAG
- [s3: The new RAG framework that trains search agents with minimal data](https://venturebeat.com/ai/s3-the-new-rag-framework-that-trains-search-agents-with-minimal-data/)
- [Ilya Rice: How I Won the Enterprise RAG Challenge](https://abdullin.com/ilya/how-to-build-best-rag/)

# Database
[Dremel: Interactive Analysis of Web-Scale Datasets](https://static.googleusercontent.com/media/research.google.com/en//pubs/archive/36632.pdf)

# Apple FM
https://machinelearning.apple.com/research/apple-foundation-models-2025-updates
# JEPA
- https://ai.meta.com/blog/yann-lecun-ai-model-i-jepa/
- https://ai.meta.com/blog/v-jepa-2-world-model-benchmarks/

